...
int main(int argc, char** argv) {
	...
	const char
			* path_img =
					cimg_option("-i",(char*)0,"Ruta y nombre de la imagen que será cargada para comprimir.");
	const char
			* path_comprimida =
					cimg_option("-o",(char*)0,"Ruta y nombre de la imagen comprimida que será guardada.");
	...
	if ((img.width() > 65536) || (img.height() > 65536)) {
		cout << "El tamaño máximo de la imagen debe ser de 65536x65536."
				<< endl;
		exit(1);
	}
	...
	if (suma != M * N) {
		...
		if (img_comp.fail()) {
			...
			for (unsigned int i = 0; i < I_comp.size(); i++) {
	...
	if (img_recons.fail()) {
		...
		while (!img_recons.eof()) {
			...
			if (!img_recons.eof()) {
				if (img_recons.fail()) {
					cerr << "Error al intentar leer la imagen comprimida"
							<< endl;
					img_recons.close();
					exit(1);
				}
	...
	cout << "MSE orig-decomp:    " << img.get_channel(0).MSE(imagen_recons)
			<< endl;
