...
//fixme: esta mal el H corregir
	CImg<double> H(img.width(), img.height(), 1, 1);
	CImgDisplay disp1, disp2;
	CImg<double> filtrada = aplicar_Gaussiano_PA_desdetiempo<double> (img, H, varianza);
	...
	while (!disp1.is_closed()) {
		...
		filtrada = aplicar_Gaussiano_PA_desdetiempo<double> (img, H, varianza);
		filtrada.display(disp1);
		disp1.set_title("imagen filtrada");
