	...
	cimg_forXY(img_segmentada,x,y)
		{
			...
			} else //esta en un radio <= a S
			{
				...
				{
					extremo_derecho = 360.0 - tol_h;
				}
				if ((H + tol_h) > 360.0) { // si es mayor a 360 grados...
					extremo_izquierdo = 0 + tol_h;
				}
				//ahora tengo que ver si esta en el rango de H que queremos....
				if (H < extremo_izquierdo && H > extremo_derecho) {//esta adentro del angulo con la tolerancia
...
cimg_forXY(img_segmentada,x,y) //recorro imagen para ver si esta dentro de la esfera el putno o fuera de la misma con un radio te tolerancia
	{
		...
		g_img = img(x, y, 0, 1);
		b_img = img(x, y, 0, 2);
		distancia = pow((r_img - R), 2) + pow((g_img - G), 2) + pow(
				(b_img - B), 2); //ecuacion de la esfera r^2=(x-x0)^2+(y-y0)^2+(z-z0)^2
