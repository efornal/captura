...
int main(int argc, char **argv) {
	...
	CImgList<float> lista(imagen_oscura_R, imagen_oscura_G, imagen_oscura_B);
	...
	/*CImgList<float> lista1(, imagen_oscura_R_eq,
	 imagen_oscura_G_eq, imagen_oscura_B_eq);*///esto lo coment porque en tendria que generar
	//la imagen RGB ya que la funcion me la normaliza y me devuelve un solo canal... y si lo muestreo
	//asi se va a ver _todo en rojo y va dar lugar a confusion

	CImgDisplay dispaux2(componer_imagen<float> (imagen_oscura_R_eq,
			imagen_oscura_G_eq, imagen_oscura_B_eq));
	//FIXME: en la imagen ecualizada con los canales RGB por serparado quizas esta mas nitida que en la que ecualize
	// sin separarlos..? la sombrilla, el edificio... pero es poca la dif.
