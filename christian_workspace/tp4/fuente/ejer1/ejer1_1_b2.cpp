...
int main(int argc, char **argv) {
	...
	cimg_forXY(HSI_H,x,y)
		{
			HSI_S_copia(x, y) = float(HSI_S(x, y)) / float(HSI_S.height()); // la fila va de 0 a 1, por tanto dividimos el pedazo en 255 valores=height
		}
	...
	cimg_forXY(HSI_S_copia,x,y) //solo lo hago para mostrar el canal este..
		{
			HSI_S_copia(x, y,0,0) *=255.0;
		}
	CImgDisplay disp4(HSI_S_copia, "Canal S variando por filas de 0 a 1", 1);
	HSI_S_copia.print(); // en realidad notar el que lso valores de S son re chicos por eso no se ven nada!
	CImgDisplay disp5(HSI2,
			"imagen HSI con S variando de 0 a 1 en filas en HSI", 0);
	CImgDisplay disp6(HSI2.get_HSItoRGB(),
			"imagen HSI con S variando de 0 a 1 en filas en RGB", 0);
