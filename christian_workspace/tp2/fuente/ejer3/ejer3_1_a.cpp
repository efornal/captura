...
int main() {
	...
	sleep(3);
	sumar <unsigned char> (imagenblanca, negativo(imagenblanca), true).display(display1);
	display1.set_title("= (suma normalizada)");
	sleep(3);
	sumar <unsigned char> (imagenblanca, negativo(imagenblanca), false).display(display1);
	...
	CImgList<float> lista1(im1, im2, sumar <float> (im1, im2, true)); //armo una lista con las imagenes y la suma de las mismas
	...
	CImgList <float> lista2(im1, im2, sumar<float> (im1, im2, false));
	...
	CImgList<float> lista3(im1, negativo(im1), sumar<float> (im1,
			negativo(im1), true));
	...
	CImgList<float> lista4(im1, negativo(im1), sumar<float>(im1,
			negativo(im1), false));
	...
	CImgList<float> lista5(im1, im1, sumar<float> (im1, im1, false));
	CImgDisplay disp5(lista5, "suma de imagenes con si misma - NO normalizado"); //muestreo en un display

	CImgList<float> lista6(im1, im1, sumar<float> (im1, im1, false));
