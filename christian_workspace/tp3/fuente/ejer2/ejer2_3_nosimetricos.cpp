...
int main(int argc, char **argv) {
	...
	CImg<float> resultado_correlacion = imagen.get_correlate(mascara3x3_no_simetrica<float>());
	CImg<float> resultado_convolucion = imagen.get_convolve(mascara3x3_no_simetrica<float>());
	...
	mascara3x3_no_simetrica<float> ().normalize(0, 255).display("mascara");
