...
int main(int argc, char **argv) {
	...
	CImg<float> mascara1 = generar_mascara_PA_suma0<float>(1).normalize(0,255);
	CImg<float> mascara2 = generar_mascara_PA_suma0<float>(2).normalize(0,1);
	CImg<float> mascara3 = generar_mascara_PA_suma0<float>(3);

	imagen.get_convolve(mascara1).display(disp2);
	disp2.set_title("mascara tipo 1 con suma 0");

	imagen.get_convolve(mascara2).display(disp3);
	disp3.set_title("mascara tipo 2 con suma 0");

	imagen.get_convolve(mascara3).display(disp4);
