...
int main(int argc, char** argv) {
  ...
  const char * path_img =
    cimg_option("-i",(char*)0,
                "Ruta y nombre de la imagen que será cargada para comprimir.");
  ...
  T.display("coeficientes para la T-coseno");
  ...
  for (int i = 0; i < nblockcols; i++) {
    for (int j = 0; j < nblockrows; j++) {
      ...
      block.display("nuevo bloque");
      dct_coef = T * block * T.get_transpose();
      dct_coef.display("dct-coef: T*bloque*Tinv.(=transpuesta)");
      int s = 0;
      for (int k = 0; k < sizeblock; k++) {
        for (int l = 0; l < sizeblock; l++) {
          s++;
  ...
  bloques.display("Coef DCT de cada bloque por columna");
  ...
  cout << "Error entre la imagen original y la comprimida: " << error
       << endl;
